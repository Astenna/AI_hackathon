...
# load inference data
inference_data = pd.read_csv('datasets/inference/prepared/inference_data.csv', sep=',',header=None).values
...
input_vector.append(np.array([inference_data[-1,7],inference_data[-2,7],inference_data[-3,7],inference_data[-4,7],inference_data[-5,7]]).reshape((5,1)))
...
def get_predict_json(date, prediction):
 ...
 data = {'date': date, 'prediction': {'increase': str(increase),'decrease': str(decrease)}}
 return data
